...
@Superthon.ar_cmd(pattern="أيقاف السورس$")
async def _(event):
    if BOTLOG:
        await event.client.send_message(
...
@Superthon.ar_cmd(pattern="أيقاف مؤقت( [0-9]+)?$")
async def _(event):
...
@Superthon.ar_cmd(pattern="الاشعارات (تشغيل|ايقاف)$")
async def set_pmlog(event):
    input_str = event.pattern_match.group(1)
    if input_str == "ايقاف":
        if gvarstatus("restartupdate") is None:
            return await edit_delete(event, "** تم تعطيل التحديثات بالفعل **")
    ...
    await edit_delete(event, "** تم تشغيل التحديثات بالفعل **")
