...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**Superthon**\n[Superthon Userbot 🧸♥](https://t.me/Superthon)\n\n"
    buttons = [
...
@Superthon.tgbot.on(InlineQuery)
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Superthon"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("DEV", "https://t.me/Superthon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./Superthon", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./Superthon", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./Superthon", "hide.txt")
            try:
            ...
            _result = main_menu()
            result = builder.article(
                title="Superthon Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب سوبرثون **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/Superthon"),
                Button.url(
                    "المطور",
                    "t.me/KaaJBot",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل سوبرثون من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[Superthon](https://t.me/Superthon)",
...
@Superthon.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@Superthon.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Superthon.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Superthon.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
