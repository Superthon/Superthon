...
class SuperthonUserBotClient(TelegramClient):
    def ar_cmd(
        self: TelegramClient,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):  # sourcery no-metrics
                ...
                try:
                    ...
                    await edit_delete(
                        check, "** هذه المجموعه تمنع ارسال الميديا هنا ،**", 10
                    )
                    ...
                    if not disable_errors:
                        ...
                        link = "[هنا](https://t.me/Superthon_support)"
            ...
            if pattern is not None:
                ...
                if edited:
                    Superthon.add_event_handler(func, events.MessageEdited(**kwargs))
                Superthon.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                Superthon.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                Superthon.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
SuperthonUserBotClient.fast_download_file = download_file
SuperthonUserBotClient.fast_upload_file = upload_file
SuperthonUserBotClient.reload = restart_script
SuperthonUserBotClient.get_msg_link = get_message_link
SuperthonUserBotClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    SuperthonUserBotClient.send_message = send_message
    SuperthonUserBotClient.send_file = send_file
    SuperthonUserBotClient.edit_message = edit_message
